...
chrome.runtime.sendMessage(
    { JDTIfunction: 'getInputIDs' },
    function (response) {
        response.data.forEach((inputID) => {
        ...
        $(document).on('click', `#${inputIDs.join(', #')}`, function (inputID) {
            ...
            if (cursorStart === cursorFinish) {
                ...
                if (selectStart) {
                    ...
                    if (selectEnd) {
                        ...
                        // This only happens when user clicks on the the closing <TI> tag. Set selectStart to null so that it wont break the keyborad functionality
                        selectStart = null;
                    }
                }
            }
            ...
            $(`#${inputID.currentTarget.id}`).keydown(function (e) {
                if (ctrlDown && e.keyCode === backtickKey) {
                    // If ctrl is pressed
                    let {
                        start: tagStartIndex,
                        end: tagEndIndex,
                    } = getAllIndexes($(this).val()); // Find all <TI> and </TI> tags in selected template.
                    if (
                        tagStartIndex.length !== 0 &&
                        tagEndIndex.length !== 0
                    ) {
                        // Works only if the selected template contains any <TI> tag
                        if (selectStart === null && selectEnd === null) {
                            // Start from first <TI>
                            var startPos = selectNextSelectionRange(
                                $(this)[0],
                                cursorStart,
                                tagStartIndex,
                                tagEndIndex
                            ...
                            // Select next <TI> set
                            if (
                                tagStartIndex.indexOf(selectStart) ===
                                    tagStartIndex.length - 1 &&
                                tagEndIndex.indexOf(selectEnd) ===
                                    tagEndIndex.length - 1
                            ) {
                                // Currently selecting the last set of <TI>, back to first set
                                ...
                                if (
                                    tagStartIndex.indexOf(selectStart) === -1 &&
                                    tagEndIndex.indexOf(selectEnd) === -1
                                ) {
                                    // Highlighted <TI> tag is modified by user. Now we need search for the next <TI>.
                                    if (cursorStart < selectStart)
                                        cursorStart = selectStart;
                                    startPos = selectNextSelectionRange(
                                        $(this)[0],
                                        cursorStart,
                                        tagStartIndex,
                                        tagEndIndex
...
function selectNextSelectionRange(
    selector,
    cursorStart,
    tagStartIndex,
    tagEndIndex
        ...
        if (issueTypeElement !== null && projectElement !== null) {
            ...
            $.each(templates, function (key, template) {
                ...
                if (
                    ...
                    !template['projects-field'] &&
                    template['issuetype-field'] === issueTypeElement.val()
                ) {
                    if (override < 3) {
                        override = 3;
                        templateText = template.text;
            ...
            if (issueTypeElement === null) {
                console.error(
                    '*** Error: Element Id "issuetype-field" not found.'
                );
            } else if (projectElement === null) {
                console.error(
                    '*** Error: Element Id "project-field" not found.'
                );
            }
        }
    });
...
function descriptionChangeEvent(changeEvent) {
    ...
    changeEvent.target.className = changeEvent.target.className.replace(
...
function observeDocumentBody(mutation) {
    if (
        document.getElementById('create-issue-dialog') !== null ||
        document.getElementById('create-subtask-dialog') !== null
    ) {
        // Only interested in document changes related to Create Issue Dialog box or Create Sub-task Dialog box.
        if (inputIDs.includes(mutation.target.id)) {
            // Only interested in select input id fields.
            ...
            isDefaultDescription(descriptionElement.value, function (result) {
                if (result) {
                    ...
                    if (
                        descriptionElement.className.indexOf(
                            'ajs-dirty-warning-exempt'
                        ) === -1
                    ) {
                        // Default template injection should not pop up dirtyDialogMessage.
