...
function loadTemplateEditor(openTemplate = null) {
            ...
            function (response) {
                if (response.status === 'success') {
                    ...
                    $.each(defaultTemplates, function (defaultIndex, template) {
                        ...
                        $.each(
                            templatesArray,
                            function (index, excludeTemplate) {
                                ...
                                if (
                                    excludeTemplate['issuetype-field'] ===
                                    template['issuetype-field']
                                ) {
                                    if (
                                        (!excludeTemplate['projects-field'] &&
                                            !template['projects-field']) ||
                                        utils.commonItemInArrays(
                                            excludeTemplate['projects-field'],
                                            template['projects-field']
                                        )
                                    ) {
                    ...
                    if (!$.isEmptyObject(validDefaultTemplates)) {
                        $('#addDefaultDropdownButton')
                            .removeClass('emptyDropdown')
                            .addClass('waves-effect waves-light');

                        $.each(validDefaultTemplates, function (key, template) {
                            var dropdownData =
                                '<li><a class="dropdownOption" href="#!" data-issueFieldType="' +
                                template['issuetype-field'] +
                                '" data-text="' +
                                template.name +
                                '">' +
                    ...
                    $('#addDefaultDropdownButton')
                        ...
                        .removeClass('waves-effect waves-light');
                    Materialize.toast(
                        'Error loading default templates please reload the extension',
                        2000,
                        'toastNotification'
                    );
                }
    ...
    chrome.runtime.sendMessage(
        { JDTIfunction: 'getToggleStatus', toggleType: 'rateClicked' },
        function (response) {
    ...
    chrome.runtime.sendMessage(
        { JDTIfunction: 'getDomains' },
        function (response) {
            if (response.data) {
                // Send a message to sandbox.html to build the domains list
                // Once the template is compiled, a 'message' event will be sent to this window with the html
                var sandboxIFrameDomains = document.getElementById(
                    'sandbox_window'
                );
    ...
    chrome.runtime.sendMessage(
        ...
        function (response) {
            if (response.data) {
                ...
                // once the template is compiled, a 'message' event will be sent to this window with the html
                var sandboxIFrameInputIDs = document.getElementById(
                    'sandbox_window'
                );
            ...
            $.each(limits, function (key, limit) {
                switch (limit) {
                    case 'all':
                        ...
                        $('#customTemplateIssueTypeField').prop(
                            'disabled',
                            true
                        );
                        $('#customTemplateProjectsField').prop(
                            'disabled',
                            true
                        );
...
$(document).ready(function () {
        ...
        chrome.runtime.sendMessage(
            ...
            function (response) {
                if (response.status === 'success') {
                    location.reload();
                    Materialize.toast(
                        'Default templates successfully loaded',
                        2000,
                        'toastNotification'
                    );
                } else {
            ...
            chrome.runtime.sendMessage(
                ...
                function (response) {
                    if (response.status === 'success') {
                        loadTemplateEditor();
                        Materialize.toast(
                            'Input IDs successfully removed',
                            2000,
                            'toastNotification'
                        );
                    } else {
    ...
    $('#rate').click(function () {
        chrome.runtime.sendMessage(
            ...
            function (response) {
                window.open(
                    'https://chrome.google.com/webstore/detail/jira-template-injector/' +
                        chrome.runtime.id +
                        '/reviews?hl=en',
                    '_blank'
                ); // Open extension user reviews page
                if (response.status !== 'success') {
                    if (response.message) {
                        Materialize.toast(
                            response.message,
                    ...
                    chrome.runtime.sendMessage(
                        ...
                        function (response) {
                            if (response.status === 'success') {
                                location.reload();
                                Materialize.toast(
                                    'Templates successfully loaded from file',
                                    2000,
                                    'toastNotification'
                                );
                            } else {
            ...
            chrome.runtime.sendMessage(
                ...
                function (response) {
                    if (response.status === 'success') {
                        location.reload();
                        Materialize.toast(
                            'Templates successfully loaded from URL',
                            2000,
                            'toastNotification'
                        );
                    } else {
    ...
    $('#addDefaultDropdownButton').click(function () {
        if ($('#addDefaultDropdownButton').hasClass('emptyDropdown')) {
            Materialize.toast(
                'All default templates have already been added',
                2000,
                'toastNotification'
            );
    ...
    $('#export').click(function () {
        if (browserType !== 'Edge') {
            ...
            chrome.tabs.create(
                { url: chrome.runtime.getURL('/html/download.html') },
                function (tab) {
                    // Tab opened.
                }
            );
            ...
            chrome.runtime.sendMessage(
                {
                    ...
                    templateIssueType: form
                        .find('[name="issueTypeField"]')
                        .val(),
    ...
    $(document).on('click', '.collapsible-header', function () {
        ...
        $('html, body').animate(
            {
                scrollTop:
                    collapsibleBody.find('[name="nameField"]').focus().offset()
                        .top - 100,
            },
            500
        );
